...
class FaceRecognitionSystem:
    ...
    def _get_dynamic_threshold(self, student_id: str) -> float:
        ...
        return self.student_thresholds.get(student_id, base_threshold)
    ...
    def verify_student(self, student_id: str, captured_image: np.ndarray) -> RecognitionResult:
        ...
        try:
            ...
            # 1. Try multi-encoding cache (JSON)
            stored_encodings = self.multiple_encodings.get(student_id, [])

            # 2. If not, try from the DB (Student.face_encoding)
            if not stored_encodings:
                try:
                    from app.models import Student
                    student_obj = Student.query.filter_by(student_id=student_id).first()
                    if student_obj and student_obj.face_encoding:
                        enc = student_obj.face_encoding
                        if isinstance(enc, str):
                            enc = [float(x) for x in enc.strip().split(",")]
                        stored_encodings = [enc]
                except Exception as e:
                    logging.error(f"Could not load encoding from DB for {student_id}: {e}")
    ...
    # ------- [No changes needed for below: batch, system info, etc.] -------
    def get_student_encoding(self, student_id: str) -> Optional[List]:
        ...
        # Optionally add DB lookup as above if desired here
        return None
    ...
    # (Keep batch processing and system info as before)
        ...
        try:
            ...
            for i, data in enumerate(image_data_list):
                ...
                if not student_id or image is None:
                    results.append({"student_id": student_id, "success": False, "message": "Missing student_id or image"})
                    continue
    ...
    def export_student_data(self, student_id: str) -> Dict:
        try:
            if student_id not in self.multiple_encodings:
                # Optionally, also allow export from DB
                from app.models import Student
                student_obj = Student.query.filter_by(student_id=student_id).first()
                if student_obj and student_obj.face_encoding:
                    encoding = student_obj.face_encoding
                    if isinstance(encoding, str):
                        encoding = [float(x) for x in encoding.strip().split(",")]
                    return {
                        "success": True,
                        "data": {
                            "student_id": student_id,
                        ...
                        "message": "Exported 1 encoding from DB"
                    }
